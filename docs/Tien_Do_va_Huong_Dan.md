...
5. **Thiết kế Database Schema, Config mẫu & Kiểu dữ liệu TypeScript:**
   - Tạo file [supabase/schema.sql](./supabase/schema.sql) định nghĩa 9 bảng dữ liệu, khóa ngoại, chỉ mục (Indexes) hiệu năng cao, chính sách Row Level Security (RLS) bảo mật và Stored Procedure Pessimistic Locking chống lỗi tồn kho.
...
8. **Cấu hình Hạ tầng Database & Storage trên Supabase Cloud (BƯỚC 1 - Hoàn thành):**
   - Khởi tạo dự án Supabase Cloud, chạy schema `supabase/schema.sql` để tạo toàn bộ 9 bảng dữ liệu, các chỉ mục (Indexes), các chính sách bảo mật RLS ngăn chặn IDOR và trigger tạo profiles tự động.
...
2. Mở mục **SQL Editor** trong Dashboard Supabase, copy nội dung file [supabase/schema.sql](./supabase/schema.sql) paste vào và chạy lệnh **Run**.
...
4. **Cấu hình Supabase Storage:**
   ...
   - Tạo chính sách RLS cho bucket `print-files` (đã có sẵn trong `supabase/schema.sql`):
