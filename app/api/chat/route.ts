  ...
  try {
    ...
    if (isMockAI) {
      ...
      if (
        ...
        responseText = '[FORWARD_TO_HUMAN] Yêu cầu của bạn đã được ghi nhận. Tôi sẽ chuyển cuộc hội thoại này sang nhân viên hỗ trợ (con người) ngay lập tức. Vui lòng đợi trong giây lát.';
      } else if (lowercaseMsg.includes('in') || lowercaseMsg.includes('print')) {
        responseText = 'Để sử dụng dịch vụ in ấn từ xa, bạn hãy chuyển sang tab "In ấn từ xa", tải tệp PDF lên, chọn cấu hình in (đen trắng/màu, khổ giấy A3/A4/A5, đóng gáy) và bấm bắt đầu in. Hệ thống sẽ tự động tính giá và mô phỏng in ấn thời gian thực.';
      } else if (lowercaseMsg.includes('mua') || lowercaseMsg.includes('cửa hàng') || lowercaseMsg.includes('shop') || lowercaseMsg.includes('store')) {
        responseText = 'PlatPrint cung cấp gian hàng bán các ấn phẩm in sẵn như Tập sách hướng dẫn hành chính, Biểu mẫu tờ khai, Báo cáo nghiên cứu công nghệ. Bạn có thể chọn mua trong mục "Gian hàng", thêm vào giỏ hàng và chọn thanh toán bằng thẻ/điểm thưởng.';
      } else if (lowercaseMsg.includes('điểm') || lowercaseMsg.includes('points') || lowercaseMsg.includes('reward')) {
        responseText = 'Với mỗi đơn hàng in ấn hoặc mua ấn phẩm, bạn sẽ được tích điểm thưởng tương ứng với giá trị đơn hàng. Bạn có thể sử dụng điểm này để giảm giá cho các đơn hàng tiếp theo (quy đổi 10 điểm = $1). Bạn có thể xem lịch sử tích điểm trong Dashboard.';
      } else {
