...
| **Bảo mật tệp tin in ấn (Private Storage)**                               | Sử dụng Public Storage Bucket và gọi trực tiếp `getPublicUrl`                             | Ngăn chặn tuyệt đối lỗ hổng IDOR trên tệp in ấn cá nhân ở tầng hạ tầng. Sử dụng Private Storage kết hợp với tạo liên kết ký tên giới hạn thời gian (`createSignedUrl` - 24h).                                 |
...
| **UX Micro-interactions (Đàn hồi tương tác)**                             | Sử dụng giao diện tĩnh mặc định, không có hiệu ứng phản hồi nhấn nút                      | Tăng tính cảm xúc và phản hồi tức thì (physical feedback) cho người dùng bằng cách áp dụng đồng bộ hiệu ứng `active:scale-*` và con trỏ chuột `cursor-pointer` trên 100% các nút bấm.                         |
...
3. Bật Realtime cho bảng in bằng cách chạy lệnh sau trong SQL Editor:
   ```sql
   alter publication supabase_realtime add table public.print_jobs;
   ```
4. Tại mục **Storage** trên Supabase, tạo một bucket tên là `print-files` ở chế độ **Riêng tư (Private)** (hệ thống sử dụng liên kết ký tên `createSignedUrl` có thời hạn 24h để đảm bảo an toàn, chống lỗ hổng IDOR).
...
- **Thắt chặt An toàn Storage:** Đóng kín Storage Bucket `print-files` thành Private, thay thế hoàn toàn `getPublicUrl` bằng `createSignedUrl` ngăn chặn triệt để tấn công IDOR truy cập tệp in ấn của người khác.
- **UX Micro-interactions:** Bổ sung đầy đủ hiệu ứng co giãn đàn hồi và thay đổi con trỏ chuột (`cursor-pointer active:scale-*`) cho tất cả các nút điều hướng và tương tác chính.
